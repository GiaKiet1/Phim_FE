<template>
    <div class="d-flex justify-content-between">
                <h4 style="color: #e0b010">DANH SÁCH THỂ LOẠI KINH DỊ</h4>
    </div>
    ...
    <div class="row row-cols-1 row-cols-sm-2 row-cols-lg-5 row-cols-xl-5 row-cols-xxl-5">                  
            <template v-for="(v,i) in listKinhDi" :key="i">
...
export default {
    ...
    methods: {
        loadKinhDi(){
            axios
                .get("http://127.0.0.1:8000/api/khach-hang/phim/data-kinhdi")
                .then((res) => {
                    this.listKinhDi = res.data.data;
                });
        }
    },
    mounted() {
